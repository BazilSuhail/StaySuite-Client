<template>
    <div class="bg-gray-100 pt-[115px] p-6 min-h-screen justify-center items-center">
      <div class="max-w-[1150px] mx-auto">
        ...
        <div v-else-if="error || bookings.length === 0" class="min-h-screen w-full flex flex-col justify-center items-center mix-blend-multiply mt-[-150px]">
        ...
        <div v-else class="grid gap-6 mx-auto sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="booking in bookings" :key="booking._id" @click="goToListing(booking.listingId)" class="overflow-hidden mx-auto w-full px-[8px] cursor-pointer bg-white border rounded-xl hover:shadow-md transition duration-200">
            <img
            ...
            <div class="ml-[53px] md:mt-[15px]">
              ...
              <div class="flex items-center space-x-2">
                ...
                <p class="font-[600] text-[13px] text-rose-800">{{ booking.guests.adults + booking.guests.children + booking.guests.infants }}</p>
              </div>
  ...
  <style scoped>
  /* You can include your existing styles here */
  </style>
