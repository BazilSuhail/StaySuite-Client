<template>
  ...
  <div v-else class="w-full overflow-x-hidden xl:px-[160px] min-h-screen p-6 bg-white">
      ...
      <div v-if="isListingPicturesModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <v-motion class="bg-white rounded-lg overflow-y-auto py-[25px] no-scrollbar h-[80vh] w-[95vw] px-[8px] space-y-[8px] flex flex-col md:w-[90vw] mx-4 md:mx-auto shadow-lg" 
          ...
          <div class="flex flex-col space-y-[8px]">
            <div v-for="(url, index) in listing.images.additionalPictures.slice(0, 4)" :key="index" class="h-[210px] sm:h-[350px]">
      ...
      <div class="relative grid w-full overflow-hidden gap-[6px] grid-cols-5 rounded-[25px]">
        <button @click="isListingPicturesModalOpen = true" class="lg:hidden absolute top-[10px] right-[15px] px-[12px] rounded-2xl bg-white text-black">See Photos</button>
        ...
        <div class="hidden h-[430px] col-span-2 md:grid grid-cols-2 grid-rows-2 gap-[6px]">
          <div v-for="(url, index) in listing.images.additionalPictures.slice(0, 4)" :key="index" class="flex items-center justify-center h-full">
      ...
      <div class="mt-[15px] mx-auto md:p-4 space-y-6 md:space-y-8 lg:grid lg:grid-cols-3 lg:gap-6 lg:space-y-0">
        <div class="lg:col-span-2 space-y-4">
          <div class="flex md:flex-row flex-col md:justify-between md:items-center space-y-4">
            <div>
              <h2 class="text-2xl font-semibold">{{ listing.property_type || '' }} In {{ listing.address.suburb || '' }}, {{ listing.address.country || '' }}</h2>
          ...
          <div class="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-5 border shadow-md px-[8px] md:px-[45px] py-[15px] rounded-[28px] space-y-2 md:space-y-0 md:flex-row md:items-center md:space-x-4">
            ...
            <button @click="showModal = true" class="flex flex-col items-center justify-center">
              <span class="text-[20px] md:text-[24px] text-rose-700 font-[600]">{{ ratingReviews.arraySize }}</span>
          ...
          <div>
            <div class="flex items-start space-x-2 text-[17px] mb-[32px]">
              <FaMedal class="text-yellow-500 mt-[12px] text-[28px] mr-[15px]" />
              <div>
            ...
            <div class="flex items-start space-x-2 text-[17px] mb-[32px]">
              <FaToilet class="text-gray-700 mt-[12px] text-[28px] mr-[15px]" />
              <div>
                <p class="font-semibold">{{ listing.bathrooms || '' }} Bath Room{{ listing.bathrooms > 1 && 's' }}</p>
                <p class="text-gray-700">Your own {{ listing.bathrooms || '' }} bathroom{{ listing.bathrooms > 1 && 's' }} in the {{ listing.property_type }} and access to shared spaces.</p>
              </div>
            </div>
  
            <div class="flex items-start space-x-2 text-[17px] mb-[32px]">
              <FaDoorOpen class="text-gray-700 mt-[12px] text-[28px] mr-[15px]" />
              <div>
                <p class="font-semibold">Amenities</p>
                <div class="flex flex-wrap items-center">
                  <p v-for="(amenity, index) in listing.amenities" :key="index" class="text-gray-800 mt-[8px] break-words px-[7px] py-[1px] text-[13px] font-[500] border-[2px] border-gray-500 rounded-xl mr-[10px] flex items-center">{{ amenity }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-4">
          <div class="border bg-white shadow-lg rounded-lg py-[30px] px-4 border-[#e7e7e7] space-y-[18px]">
            <div class="flex justify-between items-center">
              <p class="text-2xl font-semibold">{{ listing.price }}</p>
              <p class="text-sm text-gray-600 font-[600]">/ night</p>
            </div>
  
            <div class="border border-[#818181] rounded-lg py-4 w-full">
